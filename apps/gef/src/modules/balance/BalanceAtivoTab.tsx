...
// ─── Helpers para derivar dados do ativo importado ───────────────────────────
...
const HorizontalBarChart = ({ detailedData, title, subtitle, icon: Icon, className }: { detailedData: any[], title: string, subtitle: string, icon: any, className?: string }) => {
  ...
  const sortedData = [...detailedData].sort((a, b) => {
    ...
    if (sortBy === 'value') {
      ...
      // 'liquidity' is the default order
      const getPriority = (item: any) => {
        const liquidityRank: Record<string, number> = {
          'Disponível': 1,
          'Créditos': 2,
          'Produto': 3,
          'Semoventes': 4,
          ...
          'Ativo Biológico': 6,
          'Cooperativa': 7,
          'Investimentos Cooperativa': 8,
          'Imobilizado': 9,
          'Benfeitorias': 10,
          'Terras': 11,
        };
        return liquidityRank[item.name] || 99;
      };
  ...
  const getBarColor = (item: any) => {
    ...
    if (groupName === 'Financeiro') return 'bg-[#3b82f6]';
    if (groupName === 'Produtos') return 'bg-[#f59e0b]';
    return 'bg-[#10b981]';
  };
...
export function BalanceAtivoTab({ viewMode = 'Detalhamento', importedTableData, columns: columnsProp, selectedColumn }: { viewMode?: string; importedTableData?: BalanceTableRow[] | null; columns?: string[]; selectedColumn?: string }) {
  ...
  // ─── KPI derivados ───────────────────────────────────────────────────────────
  const ativoCirc    = findGroup(activeTableData, ['circulante'], ['nao', 'não']);
  const ativoNaoCirc = findGroup(activeTableData, ['nao', 'circulante']) ?? findGroup(activeTableData, ['não', 'circulante']);
  const totalAtivo      = activeTableData.reduce((s, r) => s + (r.values[colIdx] ?? 0), 0);
  const ativoCircTotal  = ativoCirc?.values[colIdx] ?? 0;
  const ativoNaoCircTotal = ativoNaoCirc?.values[colIdx] ?? 0;
  const totalAtivoPrev    = prevIdx >= 0 ? activeTableData.reduce((s, r) => s + (r.values[prevIdx] ?? 0), 0) : 0;
  const ativoCircPrev     = prevIdx >= 0 ? (ativoCirc?.values[prevIdx] ?? 0) : 0;
  const ativoNaoCircPrev  = prevIdx >= 0 ? (ativoNaoCirc?.values[prevIdx] ?? 0) : 0;

  // ─── Dados dos gráficos de barra derivados ───────────────────────────────────
  const ativoCircBarData    = deriveBarData(ativoCirc,    colIdx);
  const ativoNaoCircBarData = deriveBarData(ativoNaoCirc, colIdx);
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-6">
        ...
        <GlassCard className="p-4 md:p-6 flex flex-col justify-between relative overflow-hidden group hover:shadow-lg transition-all duration-300 border-slate-200/60">
          ...
          <div className="flex flex-col items-start mt-4">
            ...
            {prevIdx >= 0 && totalAtivoPrev > 0 && (
              <PercentChange prev={totalAtivoPrev} curr={totalAtivo} label="em relação ao balanço anterior" />
            )}
          </div>
        </GlassCard>
        ...
        <GlassCard className="p-4 md:p-6 flex flex-col justify-between relative overflow-hidden group hover:shadow-lg transition-all duration-300 border-slate-200/60">
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <div className="flex flex-col">
              <span className="text-slate-800 font-semibold text-lg">Ativo Circulante</span>
            </div>
          </div>

          <div className="flex flex-col items-start mt-4">
            <span className="text-2xl lg:text-3xl font-bold text-slate-800 tracking-tight">
              {formatCurrency(ativoCircTotal)}
            </span>
            {prevIdx >= 0 && ativoCircPrev > 0 && (
              <PercentChange prev={ativoCircPrev} curr={ativoCircTotal} label="em relação ao balanço anterior" />
            )}
          </div>
        </GlassCard>
        ...
        <GlassCard className="p-4 md:p-6 flex flex-col justify-between relative overflow-hidden group hover:shadow-lg transition-all duration-300 border-slate-200/60">
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <div className="flex flex-col">
              <span className="text-slate-800 font-semibold text-lg">Ativo Não Circulante</span>
            </div>
          </div>

          <div className="flex flex-col items-start mt-4">
            <span className="text-2xl lg:text-3xl font-bold text-slate-800 tracking-tight">
              {formatCurrency(ativoNaoCircTotal)}
            </span>
            {prevIdx >= 0 && ativoNaoCircPrev > 0 && (
              <PercentChange prev={ativoNaoCircPrev} curr={ativoNaoCircTotal} label="em relação ao balanço anterior" />
            )}
          </div>
        </GlassCard>
      </div>
      ...
      {viewMode === 'Detalhamento' && (
        <div className="flex flex-col gap-4 lg:gap-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
            <HorizontalBarChart
              detailedData={ativoCircBarData}
              title="Ativo Circulante"
              subtitle="(Ativo Circulante | % do Total Ativo Circulante)"
              icon={DollarSign}
              className="h-full"
            />
            <HorizontalBarChart
              detailedData={ativoNaoCircBarData}
              title="Ativo Não Circulante"
              subtitle="(Ativo Não Circulante | % do Total Ativo Não Circulante)"
              icon={Home}
              className="h-full"
            />
          </div>
