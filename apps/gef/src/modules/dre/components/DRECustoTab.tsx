  ...
  return (
    <motion.div initial={{ opacity: 0, x: -6 }} animate={{ opacity: 1, x: 0 }}
      ...
      className={cn('flex items-center justify-between p-3 rounded-xl',
        highlight ? 'bg-destructive/5 border border-destructive/20' : 'bg-accent/60')}>
      <div className={cn('flex items-center gap-2 text-xs font-medium',
        highlight ? 'text-destructive font-bold' : 'text-muted-foreground')}>
        <span>{icon}</span>{label}
      </div>
      <span className={cn('text-xs font-bold', highlight ? 'text-sm text-destructive font-black' : 'text-foreground')}>{value}</span>
    </motion.div>
  );
}
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
        ...
        <motion.div className="lg:col-span-3"
          ...
          <GlassCard className="p-5 h-full hover:shadow-card transition-all duration-300">
            <h3 className="text-sm font-bold text-foreground mb-0.5">Custo por Cultura</h3>
            <p className="text-xs text-muted-foreground mb-4">Custo total desembolsado por atividade</p>
        ...
        <motion.div className="lg:col-span-2"
          ...
          <GlassCard className="p-5 h-full hover:shadow-card transition-all duration-300">
            <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-4">Composição do Custo</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
        ...
        <motion.div className="lg:col-span-3"
          initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.15 }}>
          <GlassCard className="p-5 h-full hover:shadow-card transition-all duration-300">
            <h3 className="text-sm font-bold text-foreground mb-0.5">Evolução do Custo</h3>
            <p className="text-xs text-muted-foreground mb-4">Custo total por safra + custo por hectare</p>
            <div className="h-56">
              <ResponsiveContainer width="100%" height="100%">
            ...
            <div className="flex items-center justify-center gap-6 mt-2">
              ...
              <div className="flex items-center gap-1.5 text-xs">
                ...
                <span className="text-muted-foreground font-medium">Custo/ha (R$)</span>
              </div>
            </div>
          </GlassCard>
        </motion.div>
        ...
        <motion.div className="lg:col-span-2"
          ...
          <GlassCard className="p-5 h-full hover:shadow-card transition-all duration-300">
            <h3 className="text-sm font-bold text-foreground mb-0.5">Custo — Orçado vs. Realizado</h3>
            <p className="text-xs text-muted-foreground mb-4">Meta de custo planejada</p>
            ...
            <h4 className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-3">Estrutura de resultado</h4>
            ...
            <table className="w-full text-sm">
                ...
                {culturaSorted.map((c, i) => {
                  ...
                  return (
                    <tr key={c.nome} className="border-b border-border/50 hover:bg-accent/60 transition-colors">
                      ...
                      <td className="px-3 py-3 font-semibold text-destructive">{fmtCompact(c.custo)}</td>
                      <td className="px-3 py-3 text-muted-foreground">R$ {fmtNum(Math.round(c.custoHa))}</td>
                      <td className="px-3 py-3">
                        <span className={cn('font-bold px-2 py-0.5 rounded-full text-xs',
                          c.margemLiquida >= 20 ? 'bg-primary/10 text-primary' :
                          c.margemLiquida >= 10 ? 'bg-warning/10 text-warning' :
                      ...
                      <td className="px-3 py-3">
                        <span className={cn('text-xs font-semibold',
                          c.variacaoMargem > 0 ? 'text-primary' : c.variacaoMargem < 0 ? 'text-destructive' : 'text-muted-foreground')}>
                          {c.variacaoMargem > 0 ? '+' : ''}{c.variacaoMargem.toFixed(1)} pp
                        </span>
                      </td>
                      <td className="px-3 py-3">
                        <span className={cn('font-bold px-2 py-0.5 rounded-full text-xs',
                          pctTotal >= 40 ? 'bg-destructive/10 text-destructive' :
                          pctTotal >= 20 ? 'bg-warning/10 text-warning' :
              ...
              <tfoot>
                <tr className="border-t-2 border-border bg-accent/60">
                  ...
                  <td className="px-3 py-3 font-black text-destructive">{fmtBRL(data.custoTotal)}</td>
                  <td className="px-3 py-3 font-bold text-foreground">R$ {fmtNum(Math.round(custoHa))}</td>
                  <td className="px-3 py-3 font-bold text-foreground">{data.margemLiquida.toFixed(1)}%</td>
