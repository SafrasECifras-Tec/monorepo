...
function HistoricoVBPChart({ dreDataRecord, safras, safraAtual }: {
  ...
  const CustomDot = (props: any) => {
    ...
    if (!payload.atual) return <circle key={payload.safra} cx={cx} cy={cy} r={4} fill="#22c55e" stroke="white" strokeWidth={2} />;
    return <circle key={payload.safra} cx={cx} cy={cy} r={7} fill="#22c55e" stroke="white" strokeWidth={2.5} />;
  };
      ...
      <GlassCard className="p-5 h-[460px] hover:shadow-md transition-all duration-300 flex flex-col">
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData} margin={{ top: 16, right: 24, left: 8, bottom: 0 }}>
              ...
              <Line
                ...
                strokeWidth={2.5}
                dot={<CustomDot />}
                activeDot={{ r: 7, fill: '#22c55e', stroke: 'white', strokeWidth: 2 }}
              />
            </LineChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="flex items-center gap-6 mt-3 pt-3 border-t border-slate-100">
          <div className="flex items-center gap-1.5 text-xs text-slate-500 font-medium">
            <span className="w-4 h-0.5 rounded bg-indigo-500" />
            VBP por safra
          </div>
          <div className="flex items-center gap-1.5 text-xs text-slate-500 font-medium">
            <span className="w-4 h-0 border-t-2 border-dashed border-amber-400" />
            Média do período
          </div>
          <div className="flex items-center gap-1.5 text-xs text-slate-500 font-medium ml-auto">
            <span className="w-3 h-3 rounded-full bg-indigo-500 ring-2 ring-white ring-offset-1" />
            Safra atual
          </div>
        </div>
      </GlassCard>
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                      ...
                      <th className="text-right pb-2 text-emerald-600">Lucro</th>
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {barCulturaData.map((row, i) => (
                      <tr key={row.nome} className="hover:bg-slate-50/60 transition-colors">
                        ...
                        <td className="py-2 text-right font-bold text-emerald-700 font-mono">{fmtCompact(row.Lucro)}</td>
                        <td className="py-2 text-right">
                          <span className={cn(
                            'text-[10px] font-bold px-1.5 py-0.5 rounded-full',
                            row.margem >= 20 ? 'bg-emerald-100 text-emerald-700'
                            : row.margem >= 10 ? 'bg-amber-100 text-amber-700'
