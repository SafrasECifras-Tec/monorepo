...
const selectClass = 'appearance-none bg-white/60 border border-slate-200/60 shadow-sm text-slate-700 hover:bg-white/80 transition-colors px-4 h-[40px] pr-10 text-sm font-medium rounded-xl outline-none cursor-pointer focus:border-emerald-500';
        ...
        <div className="flex flex-col gap-1.5 relative" ref={tipoRef}>
          ...
          <div className="relative">
            <button
              ...
              className="w-44 bg-white/60 border border-slate-200/60 shadow-sm text-slate-700 hover:bg-white/80 transition-colors px-4 h-[40px] text-sm font-medium rounded-xl outline-none cursor-pointer focus:border-emerald-500 flex items-center justify-between"
            >
              <span className="truncate pr-2">
                {selectedTipos.length === tiposOptions.length ? 'Todos' : selectedTipos.length === 0 ? 'Nenhum' : `${selectedTipos.length} selecionados`}
              </span>
              <ChevronDown className={cn('h-4 w-4 text-slate-500 transition-transform', isTipoOpen ? 'rotate-180' : '')} />
            </button>

            {isTipoOpen && (
              <div className="absolute top-full left-0 mt-2 w-48 bg-[#1e293b] rounded-xl shadow-lg border border-slate-700 overflow-hidden z-50">
                <div className="p-2 flex flex-col gap-1">
                  <label className="flex items-center gap-2 px-2 py-1.5 hover:bg-slate-700/50 rounded-lg cursor-pointer text-slate-200 text-sm transition-colors">
                    <input type="checkbox" checked={selectedTipos.length === tiposOptions.length && tiposOptions.length > 0} onChange={handleSelectAll} className="rounded border-slate-500 bg-slate-800 text-emerald-500 focus:ring-emerald-500/20" />
                  ...
                  {tiposOptions.map(tipo => (
                    <label key={tipo} className="flex items-center gap-2 px-2 py-1.5 hover:bg-slate-700/50 rounded-lg cursor-pointer text-slate-200 text-sm transition-colors">
                      <input type="checkbox" checked={selectedTipos.includes(tipo)} onChange={() => handleTipoToggle(tipo)} className="rounded border-slate-500 bg-slate-800 text-emerald-500 focus:ring-emerald-500/20" />
                      <span>{tipo}</span>
                    </label>
                  ))}
                </div>
