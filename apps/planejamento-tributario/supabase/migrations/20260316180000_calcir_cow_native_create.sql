-- ============================================================
-- CALCIR COW: criação nativa de itens de cenário (sem shared)
-- - Permite receitas/despesas exclusivas do cenário
-- - Mantém compatibilidade com fluxo COW de update/delete
-- ============================================================

alter table public.calcir_receitas_overrides
  alter column source_receita_id drop not null;

alter table public.calcir_despesas_overrides
  alter column source_despesa_id drop not null;

alter table public.calcir_receitas_overrides
  add column if not exists tipo text check (tipo in ('projecao', 'realizado'));
...
begin
  ...
  if new.cliente_id is distinct from v_cliente_analise then
    raise exception 'cliente_id inconsistente em calcir_receitas_overrides';
  end if;

  if new.source_receita_id is not null then
    select cliente_id
    ...
    if new.cliente_id is distinct from v_cliente_source then
      raise exception 'cliente_id inconsistente em calcir_receitas_overrides';
    end if;
  end if;
...
begin
  ...
  if new.source_despesa_id is not null then
    select cliente_id
      into v_cliente_source
...
begin
  ...
  select
    ...
    o.id,
    o.cliente_id,
    coalesce(o.tipo, 'projecao') as tipo,
    o.produto,
    o.obs,
    o.entidade,
    o.pis_cofins,
    ...
    null::text as source_modulo,
    null::text as source_documento,
    null::text as source_row_hash
  from public.calcir_receitas_overrides o
  where o.cliente_id = p_cliente_id
    and o.analise_id = p_analise_id
    and o.source_receita_id is null
...
begin
  ...
  where o.cliente_id = p_cliente_id
    and o.analise_id = p_analise_id
    and o.source_despesa_id is null
    and o.is_deleted = false;
end;
$$;
...
begin
  ...
  where o.cliente_id = p_cliente_id
    and o.analise_id = p_analise_id
    and (
      o.source_receita_id = p_source_receita_id
      or (o.source_receita_id is null and o.id = p_source_receita_id)
    )
  for update;
  ...
  if not found then
    ...
    values (
      ...
      p_analise_id,
      p_source_receita_id,
      case when p_patch ? 'tipo' then (p_patch ->> 'tipo') else null end,
    ...
    update public.calcir_receitas_overrides o
       set tipo = case when p_patch ? 'tipo' then (p_patch ->> 'tipo') else o.tipo end,
           produto = case when p_patch ? 'produto' then (p_patch ->> 'produto') else o.produto end,
...
begin
  ...
  where o.cliente_id = p_cliente_id
    and o.analise_id = p_analise_id
    and (
      o.source_despesa_id = p_source_despesa_id
      or (o.source_despesa_id is null and o.id = p_source_despesa_id)
    )
  for update;
...
create or replace function public.calcir_create_receita_override(
  p_cliente_id uuid,
  p_analise_id uuid,
  p_item jsonb
)
...
begin
  ...
  values (
    case when p_item ? 'id' then (p_item ->> 'id')::uuid else gen_random_uuid() end,
    p_cliente_id,
    p_analise_id,
    null,
    case when p_item ? 'tipo' then (p_item ->> 'tipo') else null end,
    case when p_item ? 'produto' then (p_item ->> 'produto') else null end,
    case when p_item ? 'obs' then (p_item ->> 'obs') else null end,
    case when p_item ? 'entidade' then (p_item ->> 'entidade') else null end,
    case when p_item ? 'pisCofins' then (p_item ->> 'pisCofins')::boolean
         when p_item ? 'pis_cofins' then (p_item ->> 'pis_cofins')::boolean
         else null end,
    case when p_item ? 'mes' then (p_item ->> 'mes') else null end,
    case when p_item ? 'quantidade' then (p_item ->> 'quantidade')::numeric else null end,
    case when p_item ? 'valorUnit' then (p_item ->> 'valorUnit')::numeric
         when p_item ? 'valor_unit' then (p_item ->> 'valor_unit')::numeric
         else null end,
    case when p_item ? 'total' then (p_item ->> 'total')::numeric else null end,
    false,
    1
  )
  ...
  return query
  select v_new_id, 1, true;
end;
$$;

create or replace function public.calcir_create_despesa_override(
  p_cliente_id uuid,
  p_analise_id uuid,
  p_item jsonb
)
...
begin
  ...
  values (
    ...
    p_analise_id,
    null,
    case when p_item ? 'descricao' then (p_item ->> 'descricao') else null end,
    case when p_item ? 'obs' then (p_item ->> 'obs') else null end,
    case when p_item ? 'totalAnoAnterior' then (p_item ->> 'totalAnoAnterior')::numeric
         when p_item ? 'total_ano_anterior' then (p_item ->> 'total_ano_anterior')::numeric
         else null end,
    case when p_item ? 'realizado' then (p_item ->> 'realizado')::numeric else null end,
    case when p_item ? 'aRealizar' then (p_item ->> 'aRealizar')::numeric
         when p_item ? 'a_realizar' then (p_item ->> 'a_realizar')::numeric
         else null end,
    case when p_item ? 'total' then (p_item ->> 'total')::numeric else null end,
    case when p_item ? 'creditoIBSCBS' then (p_item ->> 'creditoIBSCBS')
         when p_item ? 'credito_ibs_cbs' then (p_item ->> 'credito_ibs_cbs')
         else null end,
    false,
    1
  )
